...
const BASE_URL = import.meta.env.VITE_API_BASE_URL || "https://registered-phylis-mstily=73b1dfa2.koveb.app";
...
export const loginUser = async (credentials: LoginCredentials): Promise<AuthResponse> => {
  ...
  const response: AxiosResponse<AuthResponse> = await apiClient.post("/api/login", credentials);
...
export const registerUser = async (userData: RegisterData): Promise<AuthResponse> => {
  ...
  const response: AxiosResponse<AuthResponse> = await apiClient.post("/api/register", userData);
